...
import {buyTollRoadCard, topUpBalance, useTollRoad} from "./util.js";

async function getTollAccess() {
  try {
    const card = await buyTollRoadCard(25)
    console.log('Beli kartu tol: ', card)
    
    const topup = await topUpBalance(card,10)
    console.log('Isi saldo kartu tol: ',topup)
    
    await useTollRoad(topup)
    console.log('Gunakan akses jalan tol')
  } catch (err) {
